import OrganizationImage from "../../../assets/images/Pantai.png";

export const dataautomation = [
  {
    id: 1,
    title: "Data Automation Research Group",
    description:
      "A global group dedicated to advancing research in data automation and machine learning, promoting innovation and collaboration among data automation experts.",
    image: OrganizationImage,
    tags: ["Committee", "Data Automation", "Research"],
    buttons: [
      {
        text: "View Committee",
        link: "https://drive.google.com/drive/folders/1-4R1VSKn6FQnBoxEx4ZwyvFTyP970WQj?usp=sharing", // Adjust the link accordingly
      },
      {
        text: "View Project",
        link: "/projects/data-automation-research-group", // Adjust the link accordingly
      },
    ],
  },
  {
    id: 2,
    title: "Automation for Good Coalition",
    description:
      "A coalition of organizations focused on using data automation to solve global challenges such as climate change, healthcare, and education.",
    image: OrganizationImage,
    tags: ["Committee", "Data Automation", "Social Impact"],
    buttons: [
      {
        text: "View Committee",
        link: "/committee/automation-for-good", // Adjust the link accordingly
      },
      {
        text: "View Project",
        link: "/projects/automation-for-good-coalition", // Adjust the link accordingly
      },
    ],
  },
  {
    id: 3,
    title: "Data Automation Ethics and Governance Forum",
    description:
      "An interdisciplinary forum focusing on the ethical implications of data automation, governance, and regulatory frameworks.",
    image: OrganizationImage,
    tags: ["Committee", "Automation Ethics", "Governance"],
    buttons: [
      {
        text: "View Committee",
        link: "/committee/data-automation-ethics-governance", // Adjust the link accordingly
      },
      {
        text: "View Project",
        link: "/projects/data-automation-ethics-governance-forum", // Adjust the link accordingly
      },
    ],
  },
  {
    id: 4,
    title: "Automation Startups and Innovation Hub",
    description:
      "An initiative to foster innovation in the data automation startup ecosystem, connecting entrepreneurs, investors, and professionals in the automation field.",
    image: OrganizationImage,
    tags: [
      "Committee",
      "Automation Startups",
      "Innovation",
      "Entrepreneurship",
      "Automation",
    ],
    buttons: [
      {
        text: "View Committee",
        link: "/committee/automation-startups-innovation", // Adjust the link accordingly
      },
      {
        text: "View Project",
        link: "/projects/automation-startups-innovation-hub", // Adjust the link accordingly
      },
    ],
  },
];

export default dataautomation;
